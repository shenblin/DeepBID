...
class Self_supervised_train_bi_scan(data.Dataset):
    """
    ...
    def __init__(self, opt):
        ...
        for im_name in list(os.walk(self.datasets_path, topdown=False))[-1][-1]:
            ...
            # print('int((whole_y-patch_y+gap_y)s/gap_y) -----> ',int((self.whole_y - self.patch_y + self.gap_y) / self.gap_y))
            # print('int((whole_x-patch_x+gap_x)/gap_x) -----> ',int((self.whole_x - self.patch_x + self.gap_x) / self.gap_x))
    ...
    def __getitem__(self, index):
        ...
        # new_img_path = os.path.join(output_path, (self.name_list[index] + 'target.tif'))
        # skimage.io.imsave(new_img_path, target, check_contrast=False)
        # aa
