...
class REDSDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_lmdb:
            ...
            img_gt_path = self.gt_root / clip_name / f'{frame_name}.tif'
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = self.lq_root / clip_name / f'{neighbor:08d}.tif'
        ...
        if self.flow_root is not None:
            ...
            for i in range(self.num_half_frames, 0, -1):
                if self.is_lmdb:
                    ...
                    flow_path = (self.flow_root / clip_name / f'{frame_name}_p{i}.tif')
            ...
            for i in range(1, self.num_half_frames + 1):
                if self.is_lmdb:
                    ...
                    flow_path = (self.flow_root / clip_name / f'{frame_name}_n{i}.tif')
                img_bytes = self.file_client.get(flow_path, 'flow')
                cat_flow = imfrombytes(img_bytes, flag='grayscale', float32=False)  # uint8, [0, 255]
                dx, dy = np.split(cat_flow, 2, axis=0)
                flow = dequantize_flow(dx, dy, max_val=20, denorm=False)  # we use max_val 20 here.
    ...
    def __getitem__(self, index):
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = self.lq_root / clip_name / f'{neighbor:08d}.tif'
                img_gt_path = self.gt_root / clip_name / f'{neighbor:08d}.tif'
