...
class Stack3D_train_Dataset(data.Dataset):
    """
    ...
    def __init__(self, opt):
        ...
        self.datasets_path_lq = opt['dataroot_lq']
        self.datasets_path_gt = opt['dataroot_gt']
        ...
        self.stack_num = len(list(os.walk(self.datasets_path_lq, topdown=False))[-1][-1])
        print('Total stack number -----> ', self.stack_num)

        ind = 0
        for im_name in list(os.walk(self.datasets_path_lq, topdown=False))[-1][-1]:

            im_dir = self.datasets_path_lq + '//' + im_name
            img_input = tiff.imread(im_dir)
            im_dir = self.datasets_path_gt + '//' + im_name
            img_target = tiff.imread(im_dir)
    ...
    def __getitem__(self, index):
        ...
        img_target = self.img_target_all[stack_index]
